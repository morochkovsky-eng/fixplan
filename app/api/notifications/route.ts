...
export async function GET() {
  ...
  const { data, error: selectError } = await admin.from("notification_events").select("*").eq("apartment_id", APARTMENT_ID).eq("recipient", "owner").contains("channels", ["in_app"]).order("created_at", { ascending: false }).limit(30);
...
export async function PATCH() {
  const { admin, error, status } = await requireApartmentAccess();
  if (!admin) return NextResponse.json({ error }, { status });
  const { error: updateError } = await admin.from("notification_events").update({ read_at: new Date().toISOString() }).eq("apartment_id", APARTMENT_ID).eq("recipient", "owner").is("read_at", null);
