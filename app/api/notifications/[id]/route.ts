...
export async function PATCH(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { error: updateError } = await admin.from("notification_events").update({ read_at: new Date().toISOString() }).eq("apartment_id", APARTMENT_ID).eq("recipient", "owner").eq("id", id);
