...
export function cleaningPayload(body: Record<string, unknown>) {
  ...
  return { row: { title, type, mode, zones, zone_results: zoneResults(body.zoneResults).filter((result) => zones.includes(result.zone)), checklist, completed_items: textArray(body.completedItems), supplies: textArray(body.supplies), scheduled_for_label: String(body.scheduledFor ?? "").trim(), cleaner, cleaner_phone: String(body.cleanerPhone ?? "").trim() || null, status, cost, notes: String(body.notes ?? "").trim() || null, owner_feedback: String(body.ownerFeedback ?? "").trim() || null, require_photo_before: body.requirePhotoBefore === true, require_photo_after: body.requirePhotoAfter === true } };
}
