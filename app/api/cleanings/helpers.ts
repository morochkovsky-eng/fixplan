import type { CleaningMode, CleaningStatus, CleaningType, CleaningZoneResult } from "@/lib/cleanings";
...
export function cleaningPayload(body: Record<string, unknown>) {
  ...
  return { row: { title, type, mode, zones, zone_results: zoneResults(body.zoneResults).filter((result) => zones.includes(result.zone)), checklist, completed_items: textArray(body.completedItems), supplies: textArray(body.supplies), scheduled_for_label: String(body.scheduledFor ?? "").trim(), cleaner, cleaner_phone: String(body.cleanerPhone ?? "").trim() || null, status, cost, notes: String(body.notes ?? "").trim() || null, require_photo_before: body.requirePhotoBefore === true, require_photo_after: body.requirePhotoAfter === true } };
}

export function serializeCleaning(row: Record<string, unknown>, request: Request) {
  const appUrl = (process.env.NEXT_PUBLIC_APP_URL ?? new URL(request.url).origin).replace(/\/$/, "");
  return { id: row.id, title: row.title, type: row.type, mode: row.mode, zones: row.zones ?? [], zoneResults: row.zone_results ?? [], checklist: row.checklist ?? [], completedItems: row.completed_items ?? [], supplies: row.supplies ?? [], scheduledFor: row.scheduled_for_label, cleaner: row.cleaner, cleanerPhone: row.cleaner_phone ?? undefined, status: row.status, cost: row.cost == null ? undefined : Number(row.cost), notes: row.notes ?? undefined, requirePhotoBefore: row.require_photo_before === true, requirePhotoAfter: row.require_photo_after === true, link: row.mode === "managed" ? `${appUrl}/cleaning/${row.guest_token}` : undefined, createdAt: row.created_at_label, completedAt: row.completed_at_label ?? undefined, photos: [] };
}
