...
export async function POST(request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: cleaning, error: cleaningError } = await admin.from("cleanings").select("apartment_id,id,cleaner,status,zones").eq("guest_token", token).eq("mode", "managed").maybeSingle();
  ...
  if (!zone || !cleaning.zones.includes(zone)) return NextResponse.json({ error: "Выберите зону из задания." }, { status: 400 });
  ...
  const { data: media, error: mediaError } = await admin.from("cleaning_media").insert({ apartment_id: cleaning.apartment_id, cleaning_id: cleaning.id, phase, zone, storage_path: storagePath, media_type: file.type, filename: file.name }).select("*").single();
  if (mediaError) {
  ...
  return NextResponse.json({ photo: { id: media.id, phase: media.phase, zone: media.zone, url: signed?.signedUrl ?? "", filename: media.filename, createdAt: media.created_at } });
}
