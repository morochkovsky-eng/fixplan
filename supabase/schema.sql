...
create table public.asset_media (
  ...
  caption text,
  created_by text,
  document_type text check (document_type is null or document_type in ('passport', 'manual', 'warranty', 'receipt', 'invoice', 'estimate', 'act', 'contract', 'scheme', 'other')),
...
create table public.utility_bills (
  ...
  note text,
  allocation text not null default 'owner' check (allocation in ('owner', 'tenant', 'split')),
  tenant_amount numeric not null default 0 check (tenant_amount >= 0 and tenant_amount <= amount),
  optional_charge_label text,
  optional_charge_amount numeric not null default 0,
  optional_charge_included boolean not null default false,
  reimbursement_status text not null default 'not_required' check (reimbursement_status in ('not_required', 'awaiting', 'received')),
  reimbursed_at_label text,
  source text not null default 'web' check (source in ('web', 'telegram_private', 'telegram_group')),
...
create table if not exists public.utility_bill_line_items (
  id uuid primary key default gen_random_uuid(), apartment_id uuid not null, utility_bill_id text not null,
  position integer not null, name text not null, unit text, volume numeric, tariff numeric,
  charge_minor bigint, recalculation_minor bigint, benefit_minor bigint, total_minor bigint,
  created_at timestamptz not null default now(),
  foreign key(apartment_id,utility_bill_id) references public.utility_bills(apartment_id,id) on delete cascade,
  unique(apartment_id,utility_bill_id,position)
);
create table if not exists public.utility_bill_meter_entries (
  id uuid primary key default gen_random_uuid(), apartment_id uuid not null, utility_bill_id text not null,
  position integer not null, resource text not null, meter_number text, previous_value numeric, current_value numeric,
  consumption numeric, unit text, tariff numeric, created_at timestamptz not null default now(),
  foreign key(apartment_id,utility_bill_id) references public.utility_bills(apartment_id,id) on delete cascade,
  unique(apartment_id,utility_bill_id,position)
);
create table if not exists public.utility_bill_optional_charges (
  id uuid primary key default gen_random_uuid(), apartment_id uuid not null, utility_bill_id text not null,
  position integer not null, label text not null, kind text, amount_minor bigint not null,
  included_in_mandatory boolean not null default false, created_at timestamptz not null default now(),
  foreign key(apartment_id,utility_bill_id) references public.utility_bills(apartment_id,id) on delete cascade,
  unique(apartment_id,utility_bill_id,position)
);
create index if not exists utility_bill_line_items_bill_idx on public.utility_bill_line_items(apartment_id,utility_bill_id);
create index if not exists utility_bill_meter_entries_bill_idx on public.utility_bill_meter_entries(apartment_id,utility_bill_id);
create index if not exists utility_bill_optional_charges_bill_idx on public.utility_bill_optional_charges(apartment_id,utility_bill_id);
