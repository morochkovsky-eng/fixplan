...
- Photos and documents are downloaded server-side. Image attachments receive a technical quality profile (dimensions, byte size, edge sharpness, contrast and exposure); the model separately reports blur/compression, small-text readability, crop, glare, perspective and darkness with evidence for critical fields. A decision uses the combined signals, not a single file-size or resolution threshold.
