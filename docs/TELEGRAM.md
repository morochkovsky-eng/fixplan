...
- Photos and documents are downloaded server-side with bounded file size and request timeouts. Image attachments receive a fail-closed technical quality profile (dimensions, a 40-megapixel/one-page decode limit, byte size, edge sharpness, contrast and exposure); the model separately reports blur/compression, small-text readability, crop, glare, perspective and darkness with evidence for critical fields. A decision uses the combined signals, not a single file-size or resolution threshold. PDFs bypass `sharp`, are structurally limited to 30 pages, and use the file-input vision path.
