...
test("keeps Telegram actions behind pairing, idempotency, and confirmation", () => {
  ...
  assert.match(utilityEvalSource, /mandatoryDueAmount/);
  assert.match(telegramAssistantSource, /moneyToMinor\(args\.mandatoryDueAmount/);
  ...
  assert.match(utilityEvalSource, /отдельный проверяемый черновик/);
  assert.match(utilityEvalSource, /openingCreditAmount/);
  assert.match(telegramAssistantSource, /optionalChargeIncluded/);
  assert.match(utilityEvalSource, /Начисление и «к оплате» — разные величины/);
  assert.match(telegramAssistantSource, /Для каждого нового платёжного документа/);
  ...
  assert.match(telegramAssistantSource, /max_output_tokens: 2200/);
  assert.match(telegramAssistantSource, /не больше шести коротких строк/);
  assert.match(telegramAssistantSource, /анализируй только табло/);
  assert.match(telegramAssistantSource, /автоматы, УЗО и щиток игнорируй/);
  assert.match(telegramAssistantSource, /обычным текстом Telegram без Markdown/);
  ...
  assert.match(assistantRepliesSource, /черновик документа/);
  assert.match(telegramWebhookSource, /Создать счёт/);
  assert.match(telegramWebhookSource, /Удалить черновик/);
  assert.match(assistantRepliesSource, /Документ сохранён отдельным черновиком/);
  assert.match(telegramWebhookSource, /Исключить страховку/);
  assert.match(assistantRepliesSource, /Начислено за период/);
...
test("supports utility bills as a first-class apartment section", () => {
  ...
  assert.match(utilityBillServiceSource, /numericAmount <= 0/);
...
test("keeps inspection and work order cards compact on mobile", () => {
  ...
  assert.match(pageSource, /variant="destructive-soft"[\s\S]*?Удалить/);
