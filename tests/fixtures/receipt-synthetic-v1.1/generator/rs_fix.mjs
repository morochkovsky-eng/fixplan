...
for (const f of fs.readdirSync(`${OUT}/oracle/_semspec`).sort()) {
  ...
  const literal = JSON.parse(fs.readFileSync(`${OUT}/oracle/literal_source/${spec.caseId}.json`, "utf8").replace(/\u2212/g, "-"));
  ...
  const slot = (rowId, s) => {
    ...
    if (s[0] === "T") return { cellIds: [cellId], tokenIds: [] };
  ...
  fs.writeFileSync(`${OUT}/oracle/_observed_minusfix_${spec.caseId}.json`, JSON.stringify({ classification, hints, problems, observed }, big, 1));
