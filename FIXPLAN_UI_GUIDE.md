...
### Обновление по авторским макетам, 10 сентября 2026

Текущая реализация следует файлу FixPlan `fSlKjQJ1qHZmA6CWrq9ZyT`, странице 48:2. Подробная сверка и ограничения: [FIGMA_SYNC_2026-09-10.md](FIGMA_SYNC_2026-09-10.md).

- Новая верхняя навигация действует также на desktop, прежний сайдбар заменён. На mobile логотип и селект квартиры видны напрямую; часы отсутствуют по 48:10, тема доступна в настройках.
- Button/Badge/Tabs используют Label Default 12/20 Medium; поля остаются 14/20 desktop и 16/24 mobile. Mobile-кнопки и табы сохраняют 13/20.
- Обновлены primary, secondary, muted, status-success-fg, status-warning-fg и asset-notice-background. Остальные цвета не заменяются новой палитрой произвольно.
- Ассистент: центральная сворачиваемая панель по 4:4151/4:4606, а не правая колонка.
- Следующие разделы от 9 сентября сохраняются как история решений. При расхождении этих пунктов с ними приоритет имеет обновление от 10 сентября; визуальная приёмка новой реализации ещё не завершена.
