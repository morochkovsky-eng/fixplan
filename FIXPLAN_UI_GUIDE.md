...
### 1.1. Что является стандартом, а что кандидатом

- **Figma Quadratic UI** — community-библиотека на основе shadcn/ui. Она рассматривается как кандидат на визуальный стандарт геометрии и состояний компонентов.
- **Geist, shadcn/ui и Vercel AI Elements** уже используются в проекте. Это отдельный источник реализации и продуктовых ограничений.
...
- До согласования сохраняется действующее требование использовать Geist/Vercel AI Elements. Конфликт шрифта Inter в Quadratic UI и Geist в FixPlan вынесен на решение, а не исправлен автоматически.
...
1. Шрифт продукта: сохранить Geist или перейти на Inter ради буквального соответствия Quadratic.
2. Базовая desktop-высота input/button/select: текущая compact `32` или Quadratic Medium `40`.
3. Card: текущие radius `14`/padding `16` или Quadratic radius `16`/padding `20–24`.
4. Dialog: текущая ширина `384` или Quadratic desktop `512`.
5. Ширины expanded/collapsed sidebar и assistant panel: отсутствуют в библиотеке, нужны продуктовые токены FixPlan.
6. Mobile navigation, bottom assistant и safe-area: отсутствуют в библиотеке, нужны правила FixPlan.
7. Стратегия длинных списков: pagination, virtualization или progressive disclosure.
